...
<div class="form-page">
    <h2>✏️ Chỉnh sửa phòng học</h2>
    
    <div class="info-box">
        <p><strong>Đang chỉnh sửa:</strong> <?= escapeHtml($roomData['name']) ?> (<?= escapeHtml($roomData['code']) ?>)</p>
    </div>
    
    <form method="POST" action="index.php?action=update" class="room-form">
        <!-- CSRF Token -->
        <input type="hidden" name="csrf_token" value="<?= escapeHtml(generateCsrfToken()) ?>">
        <input type="hidden" name="id" value="<?= escapeHtml($roomData['id']) ?>">
